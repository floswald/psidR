...
#' #
#' d1 <- build.panel(datadir="~/datasets/PSID/fam-files",fam.vars=famvars1,ind.vars=indvars1,design="all")	
#' d2 <- build.panel(datadir="~/datasets/PSID/fam-files",fam.vars=famvars2,ind.vars=indvars2,design="balanced",verbose=TRUE)	
#' d3 <- build.panel(datadir="~/datasets/PSID/fam-files",fam.vars=famvars3,ind.vars=indvars3,heads.only=FALSE,core=FALSE)	
#' d4 <- build.panel(fam.vars=famvars3,SAScii=TRUE,verbose=TRUE)	# with SAScii=TRUE data.dir not required
#' # 
...
#' #
#' famvars4 = data.frame(year=c(2001,2003),house.value=c(NA,"ER21043"),total.income=c("ER20456","ER24099"),education=c("ER20457","ER24148"))
#' d <- build.panel(datadir="~/datasets/PSID/from-SAS/",fam.vars=famvars4,design="all",SAScii=TRUE)	# will store .rda dataframes into datadir
#' } 
#' # ######################################
...
#' # 
#' # for sake of illustration, suppose the PSID has a total of 2N people (i.e. N are neither in year1 nor year2, but in some other years)
#' IND2009ER <- data.frame(ER30001=sample((2*N):(4*N),size=2*N),ER30002=sample(1:(2*N),size=2*N))
#' 
#' # if a person is observed, they have an interview number in both years. if not observed, it's zero. randomly allocate persons to ER30001.
...
#' # set heads to FALSE to have a clear count. data will contain column "relation.head" holding the relationship code.
#' d <- build.panel(datadir=my.dir,fam.vars=famvars,ind.vars=indvars,core=FALSE,heads=FALSE,verbose=TRUE)	
#'
...
#' # only the ones who are heads in BOTH years are present (since design='balanced' by default)
#' d <- build.panel(datadir=my.dir,fam.vars=famvars,ind.vars=indvars,core=FALSE,heads=TRUE,verbose=FALSE)	
#' print(d$data[order(pid)],nrow=Inf)
#' 
#' # change sample design to "all": 
#' # we'll keep individuals if they are head in one year, and drop in the other
#' d <- build.panel(datadir=my.dir,fam.vars=famvars,ind.vars=indvars,core=FALSE,heads=TRUE,verbose=FALSE,design="all")	
#' print(d$data[order(pid)],nrow=Inf)
#' 
